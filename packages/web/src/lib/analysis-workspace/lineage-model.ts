...
export interface LineageGraphNodeLayout {
  ...
  column: number;
  depth: number;
  side: 'downstream' | 'selected' | 'upstream';
  passCount: number;
  failCount: number;
...
export function resolveLineageSide(
  ...
  upstreamMap: Map<string, number>,
  downstreamMap: Map<string, number>,
): { side: 'downstream' | 'selected' | 'upstream'; depth: number } {
...
): { column: number; resources: ResourceNode[] }[] {
...
): Pick<LineageGraphModel, 'graphEdges' | 'nodeLayouts'> {
