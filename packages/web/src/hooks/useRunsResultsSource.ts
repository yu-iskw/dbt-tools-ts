...
type WorkerResponse = WorkerReadyMessage | WorkerQueryResultMessage | WorkerErrorMessage;
...
export function useRunsResultsSource(
  ...
  useEffect(() => {
    ...
    const worker = new Worker(new URL('../workers/resultsQuery.worker.ts', import.meta.url), {
      type: 'module',
    });
