...
This page covers the risks and guardrails for using dbt-tools with coding agents through MCP or agent skills.

## How the MCP boundary works

The MCP server (`@dbt-tools/mcp`) is a tool surface that exposes parsed artifact data to a coding agent. The architecture is:
...
The coding agent controls what happens next. The agent may:
...
**dbt-tools does not control what a coding agent does with tool responses.**

## Risks to understand

**Metadata exposure:** Artifact files may contain model names, column names, error messages, execution timing, project configuration, and environment metadata. Everything the MCP server returns from these files may appear in the coding agent's context.
...
**Client data handling:** Some coding agents log tool responses, send them to a cloud API for inference, or include them in training data depending on the subscription plan and privacy settings. Review your coding agent's data handling policy before connecting it to production artifacts.
...
**Review coding agent privacy settings.** Before connecting the MCP server to a cloud coding agent, review whether the agent sends tool responses to the provider's inference API, whether it logs sessions, and what the provider's data retention policy is.

**Scope what the agent can ask.** Agent skills from [`plugins/dbt-tools-cli`](https://github.com/yu-iskw/dbt-tools-ts/blob/main/plugins/dbt-tools-cli/README.md) and [`plugins/dbt-tools-mcp`](https://github.com/yu-iskw/dbt-tools-ts/blob/main/plugins/dbt-tools-mcp/README.md) provide named operations with predictable inputs and outputs. They are safer than open-ended prompts that ask the agent to run arbitrary CLI commands.
...
- Passing production artifact sessions to shared or untrusted coding agents
