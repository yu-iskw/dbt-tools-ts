...
![skills invoke CLI or MCP; Web, CLI, and MCP each use the shared analysis library; Web is not driven by CLI or MCP.](/diagrams/surface-routing.svg)

Skills invoke CLI or MCP only. Web shares the same analysis library (`@dbt-tools/core`) and is not driven by CLI or MCP.
