...
| ----------------------- | -------- | ----------------------------------------------------------------------------------------------- |
| `DBT_TOOLS_S3_REGION`   | No\*     | AWS region of the bucket. Recommended when the SDK cannot infer it. Falls back to `AWS_REGION`. |
| `DBT_TOOLS_S3_ENDPOINT` | No       | Custom S3-compatible endpoint URL                                                               |

\*Set `DBT_TOOLS_S3_REGION` (or `AWS_REGION`) when the AWS SDK needs an explicit region—for example some S3-compatible endpoints, or environments without a default region in the credential chain.

Standard AWS credential variables also apply: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `AWS_PROFILE`, and `AWS_REGION`. dbt-tools inherits the standard AWS SDK credential chain.
